...
Developed with Claude Code, updated with Cursor/Codex 5.3, further modified with GitHub Copilot and Claude Sonnet 4.6
"""
...
# Optional local LLM offloading (OpenAI-compatible endpoint)
...
# Normalize root logger records to catalyst_center_core in shared log file.
class _CatalystCoreLogNameFilter(logging.Filter):
    def filter(self, record: logging.LogRecord) -> bool:
        if record.name == "root":
            record.name = "catalyst_center_core"
        return True


_catalyst_log_handler = logging.FileHandler(os.path.join(PATH, "logs", "catalyst_center_mcp.log"), mode="a")
_catalyst_log_handler.addFilter(_CatalystCoreLogNameFilter())

# Configure logging
logging.basicConfig(
    level=logging.INFO,
    format="%(asctime)s - PID:%(process)d - %(name)s - %(levelname)s - %(message)s",
    handlers=[_catalyst_log_handler],
    force=True
)

logging.info("=" * 80)
logging.info("Catalyst MCP Server with API Explorer Started")
...
class CatalystInventoryCache:
    """Per-cluster in-memory cache for inventory lookups (devices, sites, fabric sites).

    Follows the same pattern as CatalystCenterTokenManager: store data with a TTL,
    serve from cache when fresh, transparently refresh when stale.

    Populated lazily on the first successful call to get_network_devices, get_sites,
    or sda/fabricSites. Consumed automatically when tools need a UUID, siteId, or
    fabricId to resolve a hostname / IP / name supplied by the user.
    """

    TTL_SECONDS = 600  # 10 minutes

    def __init__(self):
        # { cluster_name: { "devices": [...], "sites": [...], "fabric_sites": [...], "ts": datetime } }
        self._store: dict = {}
    ...
    def _entry(self, cluster: str) -> dict:
        return self._store.setdefault(cluster, {"devices": [], "sites": [], "fabric_sites": [], "ts": None})

    def _is_stale(self, cluster: str) -> bool:
        ts = self._store.get(cluster, {}).get("ts")
        if not ts:
            return True
        return (datetime.now() - ts).total_seconds() > self.TTL_SECONDS

    def _touch(self, cluster: str):
        self._entry(cluster)["ts"] = datetime.now()

    # ------------------------------------------------------------------
    # Population (called after successful API responses)
    # ------------------------------------------------------------------

    def store_devices(self, cluster: str, response_list: list):
        """Cache raw device objects from get_network_devices response."""
        entry = self._entry(cluster)
        entry["devices"] = response_list
        self._touch(cluster)
        logging.debug(f"[InventoryCache] Stored {len(response_list)} devices for cluster '{cluster}'")

    def store_sites(self, cluster: str, response_list: list):
        """Cache raw site objects from get_sites response."""
        entry = self._entry(cluster)
        entry["sites"] = response_list
        self._touch(cluster)
        logging.debug(f"[InventoryCache] Stored {len(response_list)} sites for cluster '{cluster}'")

    def store_fabric_sites(self, cluster: str, response_list: list):
        """Cache raw fabric site objects from sda/fabricSites response."""
        entry = self._entry(cluster)
        entry["fabric_sites"] = response_list
        self._touch(cluster)
        logging.debug(f"[InventoryCache] Stored {len(response_list)} fabric sites for cluster '{cluster}'")

    # ------------------------------------------------------------------
    # Resolution helpers (called before building API requests)
    # ------------------------------------------------------------------

    def resolve_device_uuid(self, identifier: str, cluster: str) -> str | None:
        """Return UUID for a device identified by hostname, management IP, or partial name."""
        if self._is_stale(cluster):
            return None
        identifier_lower = identifier.lower()
        for dev in self._entry(cluster)["devices"]:
            if (dev.get("id") == identifier or
                    (dev.get("hostname") or "").lower() == identifier_lower or
                    dev.get("managementAddress") == identifier or
                    identifier_lower in (dev.get("hostname") or "").lower()):
                return dev.get("id")
        return None

    def resolve_site_id(self, name: str, cluster: str) -> str | None:
        """Return siteId for a site matched by name or name fragment."""
        if self._is_stale(cluster):
            return None
        name_lower = name.lower()
        for site in self._entry(cluster)["sites"]:
            site_name = (site.get("name") or site.get("nameHierarchy") or "").lower()
            if name_lower == site_name or name_lower in site_name:
                return site.get("id")
        return None

    def get_fabric_site_ids(self, cluster: str) -> list[str]:
        """Return all fabric site IDs (for sda/fabricDevices fan-out)."""
        if self._is_stale(cluster):
            return []
        return [fs.get("id") for fs in self._entry(cluster)["fabric_sites"] if fs.get("id")]

    def get_all_device_uuids(self, cluster: str) -> list[str]:
        """Return all device UUIDs (useful for bulk CLI operations)."""
        if self._is_stale(cluster):
            return []
        return [dev.get("id") for dev in self._entry(cluster)["devices"] if dev.get("id")]

    def invalidate(self, cluster: str | None = None):
        """Force a cache refresh on next access (all clusters, or a specific one)."""
        if cluster:
            if cluster in self._store:
                self._store[cluster]["ts"] = None
        else:
            for key in self._store:
                self._store[key]["ts"] = None
        logging.info(f"[InventoryCache] Invalidated cache for: {cluster or 'all clusters'}")

    def summary(self) -> dict:
        """Return a human-readable cache status dict."""
        result = {}
        for cluster, entry in self._store.items():
            ts = entry.get("ts")
            age = int((datetime.now() - ts).total_seconds()) if ts else None
            result[cluster] = {
                "devices": len(entry["devices"]),
                "sites": len(entry["sites"]),
                "fabric_sites": len(entry["fabric_sites"]),
                "age_seconds": age,
                "stale": self._is_stale(cluster),
            }
        return result
...
class EnhancedDeclarativeCatalystServer:
    def __init__(self):
        logging.info("Initializing Catalyst MCP server...")
        ...
        self.analytics_file = f"{PATH}/logs/catalyst_center_mcp_explorer_analytics.json"
        ...
        # Inventory cache — lazily populated, used to resolve names → UUIDs/IDs
        self.inventory_cache = CatalystInventoryCache()
        ...
        self._setup()
        logging.info("Catalyst MCP server ready!")
    ...
    def _setup(self):
        ...
        logging.info("Setting up Catalyst MCP server handlers...")
        ...
        async def call_tool(name: str, arguments: dict):
            ...
            try:
                ...
                elif name == "invalidate_inventory_cache":
                    self.inventory_cache.invalidate(arguments.get("cluster"))
                    return {'content': [{'type': 'text', 'text': json.dumps(
                        {"status": "ok", "message": "Inventory cache invalidated",
                         "cache": self.inventory_cache.summary()}, indent=2)}]}
        ...
        logging.info("Catalyst MCP server handlers registered")
        ...
        if method and method.upper() != "GET":
            return {
                "content": [{
                    "type": "text",
                    "text": json.dumps({
                        "error": "Security restriction: API explorer indexes and executes GET endpoints only",
                        "provided_method": method.upper(),
                        "allowed_methods": ["GET"],
                        "reason": "POST/PUT/DELETE/PATCH are intentionally excluded from explorer auto-discovery"
                    }, indent=2)
                }]
            }
    ...
    async def _execute_on_clusters_with_analytics(self, path: str, method: str, parameters: dict, cluster_param: str):
            ...
            try:
                ...
                async with httpx.AsyncClient(verify=False, timeout=30.0) as client:
                    ...
                    if response.status_code in [200, 201, 202]:
                        ...
                        # Cache population for fabric sites discovered via the explorer
                        _resp_list = result.get("response", []) if isinstance(result, dict) else []
                        if isinstance(_resp_list, list) and _resp_list:
                            if "/sda/fabricSites" in path and not any(
                                    seg in path for seg in ["count", "/"[1:]]):
                                self.inventory_cache.store_fabric_sites(cluster_name, _resp_list)
        ...
        return await self._maybe_summarize(path, final_result)

    async def _maybe_summarize(self, context_name: str, raw_result: dict) -> dict:
        """Summarize raw API result via a local OpenAI-compatible LLM if LOCAL_LLM_URL is configured.
        Falls back silently to raw JSON if the LLM is unavailable or not configured."""
        raw_json = json.dumps(raw_result, indent=2)

        if not LOCAL_LLM_URL:
            return {'content': [{'type': 'text', 'text': raw_json}]}

        # Truncate very large payloads to stay within typical LLM context limits
        max_input_chars = 12000
        truncated = len(raw_json) > max_input_chars
        llm_input = raw_json[:max_input_chars] + ("\n... [truncated]" if truncated else "")
        ...
        try:
            llm_endpoint = LOCAL_LLM_URL.rstrip('/') + '/chat/completions'
            async with httpx.AsyncClient(timeout=60.0) as client:
                response = await client.post(llm_endpoint, json=payload, headers=headers)
                response.raise_for_status()
                summary = response.json()['choices'][0]['message']['content']

            logging.info(f"LLM summarization successful for: {context_name}")
            note = f"\n\n---\n_Summarized by local LLM ({LOCAL_LLM_MODEL})._"
            if truncated:
                note += " _Response was truncated before summarization._"
            return {'content': [{'type': 'text', 'text': summary + note}]}

        except Exception as e:
            logging.warning(f"LLM summarization failed for '{context_name}', returning raw JSON: {e}")
            return {'content': [{'type': 'text', 'text': raw_json}]}
    ...
    async def _handle_declarative_tool(self, name: str, arguments: dict):
        ...
        # ------------------------------------------------------------------
        # Cache resolution: fill in missing UUID/ID params from inventory cache
        # when a single cluster is targeted (cache is per-cluster)
        # ------------------------------------------------------------------
        if len(target_clusters) == 1:
            _cluster_name = target_clusters[0]['name']

            # deviceId path param (get_device_interfaces)
            if "deviceId" in tool_config.get("parameters", {}) and "deviceId" not in api_arguments:
                _hint = arguments.get("deviceId") or arguments.get("hostname") or arguments.get("managementAddress")
                if _hint:
                    _uuid = self.inventory_cache.resolve_device_uuid(_hint, _cluster_name)
                    if _uuid:
                        api_arguments["deviceId"] = _uuid
                        logging.info(f"[InventoryCache] Resolved deviceId '{_hint}' → {_uuid}")

            # deviceUuids body param (run_read_only_commands, get_config_changes)
            if name in ("run_read_only_commands", "get_config_changes"):
                raw_uuids = api_arguments.get("deviceUuids", [])
                resolved = []
                for item in raw_uuids:
                    if len(item) == 36 and item.count("-") == 4:
                        resolved.append(item)  # already a UUID
                    else:
                        _uuid = self.inventory_cache.resolve_device_uuid(item, _cluster_name)
                        resolved.append(_uuid if _uuid else item)
                if resolved:
                    api_arguments["deviceUuids"] = resolved
            ...
            try:
                ...
                async with httpx.AsyncClient(verify=False, timeout=30.0) as client:
                    ...
                    if response.status_code in [200, 202]:
                        ...
                        # --------------------------------------------------
                        # Cache population: store inventory data after success
                        # --------------------------------------------------
                        _response_list = result.get("response", []) if isinstance(result, dict) else []
                        if isinstance(_response_list, list) and _response_list:
                            if name == "get_network_devices":
                                self.inventory_cache.store_devices(cluster_name, _response_list)
                            elif name == "get_sites":
                                self.inventory_cache.store_sites(cluster_name, _response_list)
                        # fabric sites come via execute_catalyst_api_endpoint, handled separately
                        # --------------------------------------------------
                    else:
        ...
        return await self._maybe_summarize(name, final_result)
